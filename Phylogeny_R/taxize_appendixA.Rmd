...
library(taxize)
library(brranching) # Allows users to query phylogenetic data (from Phylomatic) using taxonomic names 
...
The [website](https://www.rdocumentation.org/packages/brranching/versions/0.7.0/topics/phylomatic) for brranching provides more detailed information. In addition, its GitHub repo provides a [vignette](https://github.com/ropensci/brranching) 
...
```{r query-phylogenetic-tree}


# Try phylomatic in brranching package 
# taxa <- c("Poa annua", "Phlox diffusa", "Helianthus annuus")
# tree <- brranching::phylomatic(taxa=taxa, get = 'POST')
# plot(tree, no.margin=TRUE)


# Try phylocomr R package as an alternative 
...
# Help documetion of ph_phylomatic provides more useful information 
# ph_tree <- ph_phylomatic(taxa = taxa_str, phylo = phylo_str) 
# plot(read.tree(text = ph_tree)) # Success! 


# Try taxize 
spnames <- c('Quercus robur', 
             'Iris oratoria', 
             ...
             'Lupinus albicaulis',
             'Pinus lambertiana')
out <- taxize::classification(spnames, db='itis') 
tr <- taxize::class2tree(out)
plot(tr)
