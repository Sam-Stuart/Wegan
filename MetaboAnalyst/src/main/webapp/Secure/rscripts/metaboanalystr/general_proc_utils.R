...
print(cls)
  ...
  #  }
    ...
    #REMOVED TO MAINTAIN USER ORDER
  ...
  # REMOVED TO KEEP CATEGORICAL DATA
  ...
  #ADDED CHECK HOW MANY VARIABLES ARE NUMERIC AND HOW MANY ARE CATEGORICAL
  ...
  minConc<-min(num.mat[num.mat>0], na.rm=T)/2;
...
#'@export
#'
RemoveMissingPercentVar <- function(mSetObj=NA, percent=perct){
...
#'replacement based on the minimum ("min), the mean ("mean"), or the median ("median") value of each variable columns,
...
ImputeVar <- function(mSetObj=NA, method="none"){
 ...
 if (method=="none" {
    ...
    msg <- c(msg,"Missing variables were replaced with the half of minimum values for each variable column.");
  }else if (method=="mean"){
    new.mat<-apply(int.mat, 2, function(x){
      if(sum(is.na(x))>0){
    ...
    msg <- c(msg,"Missing variables were replaced with the mean value for each variable column.");
    ...
    msg <- c(msg,"Missing variables were replaced with the median for each variable column.");
  }else{
    if(method == "knn"){
...
ClearNegatives <- function(mSetObj=NA, method="abs"){
  ...
  #TODO: REMOVE FUNCTIONALITY FOR WEGAN
...
FilterVariable <- function(mSetObj=NA, filter, qcFilter, rsd){
  ...
  if(filter == "none" && feat.num <= 4000){ # only allow for less than 4000
    ...
    if(var.num < 250){ # reduce 5%
      ...
      msg <- paste(msg, "Further variable filtering based on", nm);
