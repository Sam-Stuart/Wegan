...
d3.json("Scatterplot.json", function (data) {
	...
	svg.append("text").text(xLabel).attr("x", width / 3).attr("y", height + 30).style("font-size", "15px")
	...
	svg.append("text").text(yLabel).attr("y", -30).attr("x", - (height / 3)).style("text-anchor", "end").attr("transform", "rotate(-90)").style("font-size", "15px")
	...
	//        create title
	var titleDiv = d3.select("#my_dataviz")
		.append("div")
		.html(title)
