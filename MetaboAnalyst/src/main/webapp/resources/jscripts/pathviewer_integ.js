...
var domainURL = ""; //local or server

function initIntegPathwayViewer() {
    domainURL = window.parent.document.getElementById("myurl").value;

    var json_dir = window.parent.document.getElementById("mydir").value;
    $.getJSON(json_dir,
            function (data) {
                renderInmexImage(data.id[0], 1);
            }
    );
    resized2();
    resize(100, 50, 50);
...
function renderInmexImage(pathname, pa) {
    //set things to default
    paView = pa;
    keggPathName = pathname;
...
function prepareInit() {
    ...
    document.getElementById('top').innerHTML = "<br/>Rendering pathway....";
    document.getElementById('top').style.visibility = 'hidden';
    document.getElementById('top').style.visibility = 'visible';
...
function loadImage() {
    ...
    document.getElementById('title').innerHTML = keggPathName + " " + keggPathLnk;
...
function fgsessionFetched(resTxt, zoompc, panXpc, panYpc) {
    ...
    if (resTxt.substr(0, 10) == "zoomImgURL") {
        ...
        if (zoomImgURL != "NA") {
            ...
            alert("Your session time is out! \n Please log out and try again");
        }
    } else {
        alert("Your session time is out! \n Please log out and try again");
    }
...
//pa = 1 for MetaboAnalyst, 0 for view
function dragPress(evt) {   //catches when the mouse is pressed on the map
    ...
    //alert("click at:" + clickAt.x + ", " +clickAt.y);
    evt = new Evt(evt);
    drag = 1;
    xd = evt.x;
    yd = evt.y;
...
function resized() {   //Adjust the viewer size to fit the iframe
    ...
    paneHeight = parseInt((frameHeight - 80) * 0.95);   //Set the size of the viewer
    ...
    document.getElementById('viewer').style.height = "" + parseInt((frameHeight - 80) * 0.95) + "px";
    document.getElementById('top').style.height = "" + parseInt((frameHeight - 80) * 0.95) + "px";
    ...
    var cliprect = "rect(0px, " + parseInt(frameWidth * 0.48) + "px, " + parseInt((frameHeight - 80) * 0.95) + "px, 0px)"; //prevent viewer overhang from being visible
...
function resized2() {   //Adjust the viewer size to fit the iframe
    ...
    paneHeight = parseInt((frameHeight - 60) * 0.95);   //Set the size of the viewer
    paneWidth = parseInt(frameWidth * 0.95);
    loadWidth = paneWidth;
    loadHeight = paneHeight;
    baseWidth = paneWidth;
    baseHeight = paneHeight;

    document.getElementById('viewer').style.width = "" + parseInt(frameWidth * 0.95) + "px";
    document.getElementById('viewer').style.height = "" + parseInt((frameHeight - 60) * 0.95) + "px";
    document.getElementById('top').style.height = "" + parseInt((frameHeight - 60) * 0.95) + "px";
    document.getElementById('top').style.width = "" + parseInt(frameWidth * 0.95) + "px";
    document.getElementById('controls').style.width = "" + parseInt(frameWidth * 0.95) + "px";
    var cliprect = "rect(0px " + parseInt(frameWidth * 0.95) + "px " + parseInt((frameHeight - 60) * 0.95) + "px 0px)"; //prevent viewer overhang from being visible
    document.getElementById('viewer').style.clip = cliprect;
    document.getElementById('top').style.visibility = 'hidden';
    document.getElementById('top').style.visibility = 'visible';  //force a redraw of the viewer
...
function detectBrowser() {
    ...
    if (navigator.appName == 'Microsoft Internet Explorer')
    {
        ...
        if (ver < 8.0) {
            alert("The application has been tested on Internet Explorer 8, " + "\n"
                    + "Firefox 3+, Google-Chrome 5+, and Safari 4. Some components may not work" + "\n"
                    + "properly with your current browser.");
        }
    }
}
