...
function initSMPDBPathwayViewer(viewMode) {
    //first check if in iframe, to fix domain URL
...
function renderImage(pathname, pa) {
...
function fgsessionFetched(resTxt, zoompc, panXpc, panYpc) {
    ...
    if (resTxt.substr(0, 10) == "zoomImgURL") {
        ...
        if (zoomImgURL != "NA") {
            ...
            alert("Failed to create a zoom-in image. If you are the Sys Admin, please make sure that the convert command is available in the PATH.");
        }
    } else {
        alert("Your session time is out! \n Please log out and try again");
    }
    ...
    for (var inx = 0; inx < rectArray.length; inx++) {   //check if this mouse click was on a node (rectangle method)
        ...
        if (clickatX > box.x1 && clickatX < box.x2 && clickatY > box.y1 && clickatY < box.y2) {
            ...
            if (paView == 1) {
                ...
                overlib(info, WRAP, STICKY, CAPTION, box.lb);  //display the overlib details
            }
        }
    }
}
