...
#'@export
#'
FC.Anal.unpaired <- function(mSetObj = NA,
                             fc.thresh = 2,
                             cmp.type = 0
                             ,group_name = "NULL") {
  ...
  sig.mat <-
    cbind(fc.all[imp.inx, drop = F], fc.log[imp.inx, drop = F])
...
FC.Anal.paired <-
  function(mSetObj = NA,
    ...
    count.thresh <-
      round(nrow(mSetObj$dataSet$norm) / 2 * percent.thresh)
...
PlotFC <-
  function(mSetObj = NA,
    ...
    if (fc$paired) {
      ylim <-
        c(-nrow(mSetObj$dataSet$norm) / 2,
          nrow(mSetObj$dataSet$norm) / 2)
      ...
      plot(
        ...
        xlim = xlim,
        ylim = ylim,
        xlab = GetVariableLabel(mSetObj$dataSet$type),
      ...
      for (i in 1:ncol(fc$fc.all)) {
        segments(
          ...
          i,
          fc$fc.all[1, i],
          col = ifelse(fc$inx.up[i], "magenta", "darkgrey"),
        ...
        segments(
          ...
          i,
          -fc$fc.all[2, i],
          col = ifelse(fc$inx.down[i], "magenta", "darkgrey"),
      ...
      if (fc$raw.thresh > 0) {
        ...
        dat1 <-
          mSetObj$dataSet$norm[as.numeric(mSetObj$dataSet$cls) == 1,]
        
        dat2 <-
          mSetObj$dataSet$norm[as.numeric(mSetObj$dataSet$cls) == 2,]
        ...
        ylims <-
          range(c(mns1, mns2, msd1.top, msd2.top, msd1.low, msd2.low))
        
        new.mns <- c(mns1, rep(NA, 5), mns2)
        
        cols <-
          c(rep("magenta", length(mns1)), rep(NA, 5), rep("blue", length(mns2)))
        
        pchs <-
          c(rep(15, length(mns1)), rep(NA, 5), rep(19, length(mns2)))
        ...
        axis(
          1,
          at = 1:length(new.mns),
          labels = c(1:length(mns1), rep(NA, 5), 1:length(mns2))
        )
...
GetFC <- function(mSetObj = NA,
  ...
  if (paired) {
    ...
    if (group_name == "NULL") {
      ...
      if (!"origMeta" %in% names(mSetObj$dataSet)) {
        ...
        if (!any(sapply(data, is.factor) |
                 sapply(input, is.character))) {
          cls <-   factor(input[, 1, drop = TRUE])
          AddErrMsg(
            "No categorical variables (required as group variable); did you input your categorical variables as numbers (for example, as hot-one coded)? Alternatively, try splitting a numeric variable of interest into categories."
          )
        } else{
          # GET CATEGORICAL COLUMNS FROM INPUT DATAFRAME
          facdata <-
            input[, sapply(input, is.factor) |
                    sapply(input, is.character), drop = FALSE]
        ...
        if (!any(sapply(metaData, is.factor) |
          ...
          facdata <-
            metaData[, sapply(metaData, is.factor) |
                       sapply(metaData, is.character), drop = FALSE]
          cls <- factor(facdata[, 1, drop = TRUE])
      ...
      if (!"origMeta" %in% names(mSetObj$dataSet)) {
        ...
        # GET FROM METADATA
        cls <-
          factor(mSetObj$dataSet$origMeta[, group_name, drop = TRUE])
      }
    ...
    G1 <-
      data[which(mSetObj$dataSet$cls == levels(mSetObj$dataSet$cls)[1]),]
    G2 <-
      data[which(mSetObj$dataSet$cls == levels(mSetObj$dataSet$cls)[2]),]
    ...
    if (mSetObj$dataSet$combined.method) {
      ...
      m1 <-
        colMeans(data[which(mSetObj$dataSet$cls == levels(mSetObj$dataSet$cls)[1]),])
      
      m2 <-
        colMeans(data[which(mSetObj$dataSet$cls == levels(mSetObj$dataSet$cls)[2]),])
    ...
    if (mSetObj$dataSet$combined.method) {
      ...
      names(fc.all) <-
        names(fc.log) <-
        colnames(mSetObj$dataSet$row.norm) # make even vectors
    }
  ...
  function(mSetObj = NA,
    ...
    if (sig.num > 0) {
      ...
      tt <- list (
        ...
        # only used for plot threshold line
        inx.imp = inx.imp,
        sig.mat = sig.mat
      )
...
PlotTT <-
  function(mSetObj = NA,
    ...
    plot(
      mSetObj$analSet$tt$p.log,
      ylab = "-log10(p)",
      xlab = GetVariableLabel(mSetObj$dataSet$type),
      main = mSetObj$analSet$tt$tt.nm,
      pch = 19,
...
Volcano.Anal <-
  function(mSetObj = NA,
    ...
    if (paired) {
      sig.var <-
        cbind(fc.all[1, ][inx.imp, drop = F], fc.all[2, ][inx.imp, drop = F], p.value[inx.imp, drop =
                                                                                        F], p.log[inx.imp, drop = F])
      
      if (pval.type == "fdr") {
        colnames(sig.var) <-
          c("Counts (up)", "Counts (down)", "p.adjusted", "-log10(p)")
        
      } else{
        colnames(sig.var) <-
          c("Counts (up)", "Counts (down)", "raw.pval", "-log10(p)")
      ...
      sig.var <-
        cbind(fc.all[inx.imp, drop = F], fc.log[inx.imp, drop = F], p.value[inx.imp, drop =
                                                                              F], p.log[inx.imp, drop = F])
...
PlotVolcano <-
  function(mSetObj = NA,
    ...
    MyHighlight <-
      rgb(t(col2rgb("magenta")), alpha = 80, maxColorValue = 255)
    
    
    if (vcn$paired) {
      xlim <-
        c(-nrow(mSetObj$dataSet$norm) / 2,
          nrow(mSetObj$dataSet$norm) / 2) * 1.2
      
      
      # merge fc.all two rows into one, bigger one win
      fc.all <-
        apply(vcn$fc.all, 2, function(x) {
          if (x[1] > x[2]) {
            return(x[1])
          } else{
            return(-x[2])
      ...
      plot(
        ...
        xlim = xlim,
        pch = 20,
        cex = ifelse(hit.inx, 1.2, 0.8),
        col = ifelse(hit.inx, MyHighlight, MyGray),
        xlab = "Count of Significant Pairs",
        ylab = "-log10(p)"
      )
      ...
      if (plotLbl & sum(lblInx, na.rm = T) > 0) {
        text.lbls <-
          substr(colnames(mSetObj$dataSet$norm)[lblInx], 1, 14) # some names may be too long
        text(
          vcn$fc.all[1, lblInx],
          vcn$p.log[lblInx],
          labels = text.lbls,
...
ANOVA.Anal <-
  function(mSetObj = NA,
    ...
    if (nonpar) {
      ...
      anova.res <-
        apply(as.matrix(mSetObj$dataSet$norm),
              2,
              kwtest,
      ...
      res <-
        unlist(lapply(anova.res, function(x) {
          c(x$statistic, x$p.value)
        }))
      
      res <-
        data.frame(matrix(res, nrow = length(anova.res), byrow = T), stringsAsFactors =
                     FALSE)
      ...
      if (sig.num > 0) {
        ...
        colnames(sig.mat) <-
          c("chi.squared", "p.value", "-log10(p)", "FDR", "Post-Hoc")
        ...
        colnames(my.mat) <-
          c("chi_squared", "pval_KW", "-log10(p)", "FDR")
      ...
      aov.res <-
        apply(as.matrix(mSetObj$dataSet$norm), 2, aof, cls = mSetObj$dataSet$cls)
      ...
      res <-
        data.frame(matrix(res, nrow = length(aov.res), byrow = T), stringsAsFactors =
                     FALSE)
      ...
      if (sig.num > 0) {
        ...
        if (post.hoc == "tukey") {
          ...
          cmp.res <-
            unlist(lapply(tukey.res, parseTukey, cut.off = thresh))
          ...
          cmp.res <-
            unlist(lapply(fisher.res, parseFisher, cut.off = thresh))
        ...
        colnames(sig.mat) <-
          c("f.value", "p.value", "-log10(p)", "FDR", post.nm)
    ...
    AddMsg(paste(
      c("A total of", sum(inx.imp), "significant features were found."),
      collapse = " "
    ))
...
PlotANOVA <-
  function(mSetObj = NA,
    ...
    plot(
      ...
      ylab = "-log10(p)",
      xlab = GetVariableLabel(mSetObj$dataSet$type),
      main = mSetObj$analSet$aov$aov.nm,
      type = "n"
    )
...
PlotCmpdView <- function(mSetObj = NA,
                         cmpdNm,
                         # group_name = "NULL",
                         format = "png",
                         dpi = 72,
  ...
  if (mSetObj$dataSet$type.cls.lbl == "integer") {
    cls <-
      as.factor(as.numeric(levels(mSetObj$dataSet$cls))[mSetObj$dataSet$cls])
  ...
  boxplot(
    mSetObj$dataSet$norm[, cmpdNm] ~ cls,
    las = 2,
...
GetSigTable.FC <- function(mSetObj = NA) {
  ...
  GetSigTable(mSetObj$analSet$fc$sig.mat,
              "fold change analysis",
...
GetAnovaLnMat <- function(mSetObj = NA) {
  ...
  as.matrix(rbind(
    c(0, mSetObj$analSet$aov$thresh),
    c(length(lod) + 1, mSetObj$analSet$aov$thresh)
  ))
...
GetSigTable.TT <- function(mSetObj = NA) {
  ...
  GetSigTable(mSetObj$analSet$tt$sig.mat,
              "t-tests",
              mSetObj$dataSet$type)
...
GetTtLnMat <- function(mSetObj = NA) {
  ...
  as.matrix(rbind(
    c(0, mSetObj$analSet$tt$thresh),
    c(length(lod) + 1, mSetObj$analSet$tt$thresh)
  ))
...
fac.names <- function(mSetObj = NA) {
  ...
  if (!"origMeta" %in% names(mSetObj$dataSet)) {
    ...
    if (!any(sapply(input, is.factor) |
             sapply(input, is.character))) {
      warning(
        "No categorical variables (required as group variable); all varnames in input provided; did you input your categorical variables as numbers (for example, as hot-one coded)? "
      )
      ...
      # GET CATEGORICAL COLUMNS FROM INPUT DATAFRAME
      facvars <-
        colnames(input[, sapply(input, is.factor) |
                         sapply(input, is.character), drop = FALSE])
    }
    ...
    if (!any(sapply(metaData, is.factor) |
      ...
      if (!any(sapply(input, is.factor) |
               sapply(input, is.character))) {
        warning(
          "No categorical variables (required as group variable); all varnames in metadata provided; did you input your categorical variables as numbers (for example, as hot-one coded)? "
        )
      ...
      ## CATEGORICAL FROM METADATA
      facvars <-
        colnames (metaData[, sapply(metaData, is.factor) |
                             sapply(metaData, is.character), drop = FALSE])
    }
  }
...
GetTtestRes <-
  function(mSetObj = NA,
           ...
           inx1 <-
             which(mSetObj$dataSet$cls == levels(mSetObj$dataSet$cls)[1])
           
           inx2 <-
             which(mSetObj$dataSet$cls == levels(mSetObj$dataSet$cls)[2])
           
           if (length(inx1) == 1 || length(inx2) == 1) {
             equal.var <-
               TRUE
             # overwrite use option if one does not have enough replicates
           }
...
GetUnivReport <- function(mSetObj = NA) {
  ...
  univStat.mat <-
    apply(as.matrix(mSetObj$dataSet$norm), 2, function(x) {
      # normality test for each group
      # ks <- ks.test(x[inx1], x[inx2]);
      if (var(x[inx1], na.rm = T) == 0 |
          var(x[inx2], na.rm = T) == 0) {
        # shapiro cannot work when all values are same
        method = ""
        ...
        method <-
          ifelse(((sw.g1$p.value <= 0.05) |
                    (sw.g2$p.value <= 0.05)), "(W)", "")
      }
      if (method == "(W)") {
        # wilcoxon test
        tmp <- try(wilcox.test(x[inx1], x[inx2], paired = paired))
        ...
        tmp <-
          try(t.test(x[inx1], x[inx2], paired = paired, var.equal = equal.var))
        ;
      }
      if (class(tmp) == "try-error") {
        return(NA)
        ...
        foldChange <-
          round(ifelse(foldChange >= 1, foldChange, (-1 / foldChange)), 2)
  ...
  # add FDR/q-value
  q.value <-
    sprintf("%.4f", p.adjust(p = as.numeric(levels(
      univStat.mat$p.value.origin
    ))[univStat.mat$p.value.origin], method = 'fdr'))
  
  univStat.mat <-
    cbind(univStat.mat[, c(1, 2, 3)], q.value, univStat.mat[, c(4, 5)], univStat.mat[, 6])
  
  names(univStat.mat)[1] <-
    paste("Mean (SD) of ", levels(mSetObj$dataSet$cls)[1], sep = '')
  
  names(univStat.mat)[2] <-
    paste("Mean (SD) of ", levels(mSetObj$dataSet$cls)[2], sep = '')
  ...
  names(univStat.mat)[6] <-
    paste(levels(mSetObj$dataSet$cls)[1],
          "/",
          levels(mSetObj$dataSet$cls)[2],
          sep = '')
  ...
  sigones <-
    which(as.numeric(as.character(univAnal.mat$p.value.origin)) <= threshp)
  
  
  sigDataSet.orig <-
    cbind(
      SampleID = rownames(mSetObj$dataSet$orig),
      Label = mSetObj$dataSet$cls,
      mSetObj$dataSet$orig[, c(sigones)]
    )
  sigDataSet.norm <-
    cbind(
      SampleID = rownames(mSetObj$dataSet$orig),
      Label = mSetObj$dataSet$cls,
      mSetObj$dataSet$norm[, c(sigones)]
    )
  
  write.table(
    sigDataSet.orig,
    file = paste("data_subset_orig_p", threshp, ".csv", sep = ''),
    append = FALSE,
    sep = ",",
    row.names = FALSE
  )
  ...
  write.table(
    sigDataSet.norm,
    file = paste("data_subset_norm_p", threshp, ".csv", sep = ''),
    append = FALSE,
    sep = ",",
    row.names = FALSE
  )
...
GetSigTable.Volcano <- function(mSetObj = NA) {
  ...
  GetSigTable(mSetObj$analSet$volcano$sig.mat,
              "volcano plot",
              mSetObj$dataSet$type)
...
LSD.test <- function(y, trt, alpha = 0.05) {
  ...
  means <-
    data.frame(means, std.err, replication = nn[, 2], LCL, UCL)
