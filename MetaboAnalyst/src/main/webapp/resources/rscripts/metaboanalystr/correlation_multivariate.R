...
if(!all(predictors2 %in% colnames(data)) ){
  ...
  mSetObj$analSet$linRegMulti$mod <- list(model = mod, model.name = model_name, model.data = model_data, response = facA, predictor = predictors2)
  
  #EXTRACT RESULTS
  fitted <- fitted(mod) #Predicted values
  summ <- summary(mod) # includes coefficients, residuals, fit 
  resid <- mod$residuals # Get the residuals 
  conf.int <- confint(mod, level = 0.95) #Conf intervals for predictor variables
  ...
  if (ncol(model_data) == 2) {
    ...
    importance <- relaimpo::calc.relimp(mod, type = "lmg", rela = TRUE)
  }
  ...
  #STORE RESULTS 
  mSetObj$analSet$linRegMulti$res <- list(summary = summ, response = facA, predictor = predtext, predtext = predtext1, model.data = model_data, equation = equation, r.squared.eq = paste("R-squared = ", r_sq), r.squared.adj.eq = paste("R-squared adjusted = ", r_sq_adj), r.squared = r_sq,  r.squared.adjusted = r_sq_adj,
     coeff.table = coeffs, residuals = resid, predicted.values = fitted, confidence.intervals = conf.int, covariance.matrix = covar, plot.rmse = plot_rmse, importance = importance, fileName = fileName) 
...
predictors <- mSetObj$analSet$linRegMulti$res$predictor
model_name <- mSetObj$analSet$linRegMulti$mod$model.name
model_data <- mSetObj$analSet$linRegMulti$mod$model.data
mod <- mSetObj$analSet$linRegMulti$mod$model
form <- mSetObj$analSet$linRegMulti$mod$formula
...
pred_fit <- predict(mod, newdata = model_data, interval = "confidence")
