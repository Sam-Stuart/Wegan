...
multi.reg.anal <- function(mSetObj=NA, 
 ...
 if(any(!colnames(data) %in% predictors2)){
  ...
  #STORE RESULTS 
  mSetObj$analSet$linRegMulti$res <- list(summary = summary, response = facA, predictors = predtext, model.data = model_data, equation = equation, r.squared.eq = paste("R-squared = ", r_sq), r.squared.adj.eq = paste("R-squared adjusted = ", r_sq_adj), r.squared = r_sq,  r.squared.adjusted = r_sq_adj,
