...
#'License: GNU GPL (>= 2)
#'@export
oa.NMDS <- function(mSetObj=NA, distance="NULL", abundance="NULL", metaData="NULL", envData="NULL", p.max.var="NULL", p.max.env="NULL") { 
  ...
  if (metaData=="NULL") { #No user uplaoded meta data
  ...
  if (envData=="NULL") { #No user uplaoded environmental data
      envData1 <- "NA"
  ...
  if (p.max.var=="NULL") { #If no p-value cutoff selected (default)
    p.max.var1 <- NULL
  } else { #If p-value cutoff selected
    p.max.var1 <- p.max.var #options are none (default), 0.05, 0.01, 0.005
  }
  
  #Fit variables to ordination plots for plotting arrows
  var.fit2D <- envfit(nmds2D, num_data1, permutations=999, p.max=p.max.var1)
  var.fit3D <- envfit(nmds3D, num_data1, permutations=999, p.max=p.max.var1)
  ...
  if (is.data.frame(envData1)==FALSE) { #If environmental data not uploaded
    ...
    if (p.max.env=="NULL") { #If no p-value cutoff selected (default)
      ...
      p.max.env1 <- p.max.env #options are none (default), 0.05, 0.01, 0.005
    }
    #Fit environmental data to ordination plots for plotting arrows
    env.fit2D <- envfit(nmds2D, envData1, permutations=999, p.max=p.max.env1)
    env.fit3D <- envfit(nmds3D, envData1, permutations=999, p.max=p.max.env1, choices=1:3)
  }
  ...
  if (is.data.frame(envData1)==FALSE) { #If environmental data not uploaded
    ...
    env.scores3D <- signif(scores(env.fit3D, display="vectors"), 5) #Calculate environment scores 
  }

  #Extract scores
  row.scores2D <- signif(scores(nmds2D, display="sites"), 5)
  row.scores3D <- signif(scores(nmds3D, display="sites"), 5)
  col.scores2D <- signif(scores(nmds2D, display="species"), 5)
  col.scores3D <- signif(scores(nmds3D, display="species"), 5)
  ...
  mSetObj$analSet$nmds$envData <- envData1
  mSetObj$analSet$nmds$var.fit2D <- var.fit2D
  mSetObj$analSet$nmds$env.fit2D <- env.fit2D
  mSetObj$analSet$nmds$var.fit3D <- var.fit3D
  mSetObj$analSet$nmds$env.fit3D <- env.fit3D
  mSetObj$analSet$nmds$p.max.env <- p.max.env1
  mSetObj$analSet$nmds$p.max.var <- p.max.var1
  mSetObj$analSet$nmds$sample.scores2D <- row.scores2D
  mSetObj$analSet$nmds$sample.scores3D <- row.scores3D
  mSetObj$analSet$nmds$var.scores2D <- col.scores2D
  mSetObj$analSet$nmds$var.scores3D <- col.scores3D
  mSetObj$analSet$nmds$env.scores2D <- env.scores2D
  mSetObj$analSet$nmds$env.scores3D <- env.scores3D
  
  #Write scores tables into working directory
  write.csv(row.scores2D, file="nmds_2D_row_scores.csv", row.names=TRUE)
  write.csv(row.scores3D, file="nmds_3D_row_scores.csv", row.names=TRUE)
  write.csv(col.scores2D, file="nmds_2D_column_scores.csv", row.names=TRUE)
  write.csv(col.scores3D, file="nmds_3D_column_scores.csv", row.names=TRUE)
  if (is.data.frame(envData1)==TRUE) {
    write.csv(env.scores2D, file="nmds_2D_environment_scores.csv", row.names=TRUE)
    write.csv(env.scores3D, file="nmds_3D_environment_scores.csv", row.names=TRUE)
  }
  ...
  sink("column_impact_on_nmds_3D.txt") 
  cat("Data columns may significantly impact NMDS\n")
  cat("\nNMDS dimension=3\n\n")  
  print(var.fit3D)
  sink()  
  ...
  sink("environment_impact_on_nmds_3D.txt") 
  cat("Environmental factors may significantly impact NMDS\n")
  cat("\nNMDS dimension=3\n\n")
  print(env.fit3D)
  sink()  
...
#'License: GNU GPL (>= 2)
#'@export
Plot.NMDS.2D <- function(mSetObj=NA, color="NULL", ellipse="NULL", var.arrows="NULL", env.arrows="NULL", sampleNames="NULL", meta_col_color="NULL", point.options="NULL", meta_col_point="NULL", imgName, format="png", dpi=72, width=NA) {
  ...
  envData <- mSetObj$analSet$nmds$envData
  input <- mSetObj$analSet$nmds$input
  var.fit2D <- mSetObj$analSet$nmds$var.fit2D
  env.fit2D <- mSetObj$analSet$nmds$env.fit2D
  ...
  par(xpd=TRUE, mar=c(8.1, 8.1, 8.1, 8.1))
  ...
  if (is.data.frame(metaData)==FALSE) { #If no meta data
    ...
    if (is.data.frame(envData)==TRUE) { #If environment data uploaded
      if (env.arrows!="NULL") { #If environment arrows slected
        plot(env.fit2D, col="blue")
      }
    }
    ...
    if (sampleNames!="NULL") { #If display data as lables
      ...
      if (point.options!="NULL") { #Engage point options
        ...
        with(metaData, legend("bottomright", inset=c(-0.3,0), legend=levels(meta_col_point_data), col="black", pch=unique(pchs), pt.bg="black", title=meta_col_point_name))
    ...
    #Legend
    with(metaData, legend("topright", inset=c(-0.2,0), legend=levels(meta_col_color_data), col=colors, pch=19, title=meta_col_color_name)) # Include legend for colors in figure   
  }
...
#'License: GNU GPL (>= 2)
#'@export
Plot.NDMS.stress <- function(mSetObj=NA, k="NULL", imgName, format="png", dpi=72, width=NA) {
  ...
  #Stress plot
  Cairo::Cairo(file=imgName, unit="in", dpi=dpi, width=w, height=h, type=format, bg="white")
  stressplot(nmds2D, main=paste0("Non-Metric Multidimensional Scaling\nDimension ", k,"\nShepherds Diagram\n"), yaxt="n")
  axis(2, las=2)
...
#'License: GNU GPL (>= 2)
#'@export
Plot.NDMS.scree <- function(mSetObj=NA, imgName, format="png", dpi=72, width=NA) {
  ...
  stress_data <- c(nmds1D$stress, nmds2D$stress, nmds3D$stress, nmds4D$stress, nmds5D$stress, nmds6D$stress)
  scree_data <- data.frame(1:6, stress_data)
  colnames(scree_data) <- c("NMDS_Dimension", "Stress")
  ...
  #Scree plot
  Cairo::Cairo(file=imgName, unit="in", dpi=dpi, width=w, height=h, type=format, bg="white")
  plot(x=scree_data$NMDS_Dimension, y=scree_data$Stress, type="l", xlim=c(1, 6), ylim=c(0, stressMax+0.02), xlab="Number of Dimensions", ylab="Stress", main="NMDS Scree Plot", yaxt="n", col="blue", lwd=2)
  points(x=scree_data$NMDS_Dimension, y=scree_data$Stress, cex=1.1, pch=19, col="blue")
  axis(2, las=2)
  dev.off()
}
...
#'#'Produce NMDS 3D ordination plot
#'@description Rotate NMDS analysis
#'@usage PlotNMDS3DScore(mSetObj=NA, imgName, format="json", inx1, inx2, inx3)
#'@param mSetObj Input name of the created mSet Object
...
#'@param color #Viridis pallete, options include "viridis" (default), "plasma" and "cividis"
#'@param var.arrows Boolean, TRUE to produce variable arrows, FALSE (default) to produce ordination plot without variable arrows
#'@param env.arrows Boolean, TRUE to produce environmental arrows, FALSE (default) to produce ordination plot without environmental arrows---option only available if environmental data uploaded
#'@param meta_col_color Meta data column to use for plotting colors, Can be user inputted where options are given to java using function meta.columns()
#'@param point.options Boolean, TRUE to turn data into points, FALSE (default) uses row names as data points
#'@param meta_col_point Meta data column to use for plotting points, Can be user inputted where options are given to java using function meta.columns()
#'@param imgName Input the image name
...
Plot.NMDS.3D <- function(mSetObj=NA, color="NULL", var.arrows="NULL", env.arrows="NULL", sampleNames="NULL", meta_col_color="NULL", point.options="NULL", meta_col_point="NULL", imgName, format="json"){
  ...
  envData <- mSetObj$analSet$nmds$envData
  row.scores3D <- mSetObj$analSet$nmds$sample.scores3D
  col.scores3D <- mSetObj$analSet$nmds$var.scores3D
  env.scores3D <- mSetObj$analSet$nmds$env.scores3D 
  ...
  sampleCoords <- data.frame(t(as.matrix(row.scores3D)))
  ...
  if (is.data.frame(metaData)==FALSE) { #If no meta data
    ...
    if (point.options=="NULL") {
      ...
      pchs <- pch_options[meta_col_point_data]
    }
    nmds3D_plot$score$point <- pchs
    
  }
  
  #Variables (columns)
  variableCoords <- data.frame(t(as.matrix(col.scores3D)))
  ...
  #Environment
  if (is.data.frame(envData)==FALSE) {
    nmds3D_plot$scoreEnv$axis <- "NA"
    nmds3D_plot$scoreEnv$xyzEnv <- "NA"
    nmds3D_plot$scoreEnv$nameEnv <- "NA"
    nmds3D_plot$scoreEnv$colorEnv <- "NA"
  } else {
    envCoords <- data.frame(t(as.matrix(env.scores3D)))
    colnames(envCoords) <- NULL
    nmds3D_plot$scoreEnv$axis <- paste("NMDS", c(1, 2, 3), sep="")
    nmds3D_plot$scoreEnv$xyzEnv <- envCoords
    nmds3D_plot$scoreEnv$nameEnv <- colnames(envData)
    nmds3D_plot$scoreEnv$colorEnv <- col2rgb("blue")
  }
