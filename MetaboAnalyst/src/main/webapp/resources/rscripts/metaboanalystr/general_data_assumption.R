...
#'@usage levene(mSetObj = NA, predText = "NULL")
#'@param mSetObj Input the name of the created mSetObj (see InitDataObjects)
#'@param predText Textbox input the variable names
#'@author Shiyang Zhao \email{shiyang1@ualberta.ca}
#'University of Alberta, Canada
#'@export
levene <- function(mSetObj = NA, predText = "NULL"){
  ...
  for (i in 1:g) {
    ...
    #name_v.list[[i]] <- data.frame(name_v)
    #name_v.data <- rbind(name_v.data, name_v.list[[i]])
    table_in <- cbind(predText, name_in.data, leve_2.data)
  }
...
ResidPlot <- function(mSetObj = NA, numA = "NULL", predText = "NULL", imgName, format="png", dpi=72, width=NA){
